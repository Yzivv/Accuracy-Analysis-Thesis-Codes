...
%% Uses an adaptive system to make sure the indeterminacy sets are sensible.
%%
%% INPUT: 
...
function approx = adaptbruteforce(a_mean,b_mean,f_mean,delta1,delta2,delta3,domain,bdcond,sol_amount,mesh_points)
    ...
    try
        ...
        while do_more
            ...
            for k = 1:mesh_points-1
                ...
                if ai(1,k) > ai(2,k) || bi(1,k) > bi(2,k) || fi(1,k) > fi(2,k) 
                    mesh_points = 2*mesh_points;
                    do_more = true;
                end
            end
            if mesh_points > 5000
                error('maximum mesh points reached');
            end
